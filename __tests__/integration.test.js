...
test('running with gulp 3.9.1', async() => {
    ...
    const input = ['yarn', ['run', `gulp`, 'example', '--gulpfile', `./GulpFile.3.9.1.js`], options];
    const output = {
        code: 0,
        signal: '',
        stdout: new RegExp(`^${yarn('example')}\\s*${using}\\s*${starting('example')}\\s*${finished('example')}\\s*${done}\\s*$`)
    }
...
test('running with gulp 4.0.0', async() => {
    ...
    const input = ['yarn', ['run', `gulp`, 'example', '--gulpfile', `./GulpFile.4.0.0.js`], options];
    const output = {
        code: 0,
        signal: '',
        stdout: new RegExp(`^${yarn('example')}\\s*${using}\\s*${starting('example')}\\s*${starting('generateManifest')}\\s*${finished('generateManifest')}\\s*${finished('example')}\\s*${done}\\s*$`)
    }
    await expect(input).toHaveMatchingSpawnOutput(output);
});